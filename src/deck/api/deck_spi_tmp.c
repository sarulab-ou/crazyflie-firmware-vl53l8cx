 ...
 *
 * deck_spi_tmp.c - Deck-API SPI communication implementation (FreeRTOS semaphore dependency removed)
 */
...
// ignored #ここでSPIモードを定義。モード3で動作させるため、CPOL=1, CPHA=1に設定
...
{
  const uint32_t timeoutCycles = SystemCoreClock / 50U; // about 20ms
  ...
  // No IRQ-based completion: use polling
  ...
  // Wait for completion (bare metal)
  while (((DMA_GetFlagStatus(SPI_TX_DMA_STREAM, SPI_TX_DMA_FLAG_TCIF) == RESET) ||
          (DMA_GetFlagStatus(SPI_RX_DMA_STREAM, SPI_RX_DMA_FLAG_TCIF) == RESET)) &&
         (timeout > 0U))
  {
    timeout--;
  }

  // Disable SPI DMA requests and streams
  SPI_I2S_DMACmd(SPI, SPI_I2S_DMAReq_Tx, DISABLE);
  SPI_I2S_DMACmd(SPI, SPI_I2S_DMAReq_Rx, DISABLE);
  DMA_Cmd(SPI_TX_DMA_STREAM, DISABLE);
  DMA_Cmd(SPI_RX_DMA_STREAM, DISABLE);

  while ((SPI_I2S_GetFlagStatus(SPI, SPI_I2S_FLAG_BSY) == SET) && (timeout > 0U))
  {
    timeout--;
  }

  bool result = (timeout > 0U);

  // Disable peripheral
  SPI_Cmd(SPI, DISABLE);

  // Cleanup transfer-complete flags for next transaction
