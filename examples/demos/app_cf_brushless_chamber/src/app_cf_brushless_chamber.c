...
/* Body-frame x/y velocity with absolute height/yaw hold -- for centering. */
static void setHoverVelSetpoint(setpoint_t *setpoint, float vxBody, float vyBody, float z, float yawDeg)
{
  ...
  while (1) {
    ...
    switch (appState) {
      ...
      case APP_HOVER: {
        ...
        /* Always body-velocity in x/y (as slam_tunnel's ADVANCE does) --
         * the centering command itself is the only x/y feedback; height and
         * yaw stay absolute. */
        setHoverVelSetpoint(&setpoint, vx, vy, takeoffHeight, holdYawDeg);
        commanderSetSetpoint(&setpoint, 3);
